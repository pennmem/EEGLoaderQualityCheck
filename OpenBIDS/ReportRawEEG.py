...
def process_raw_signals(sub, exp, sess, bids_root, out_path, localization=None, montage=None): 
    ...
    if is_ieeg:
        ...
        results = compare_eeg_sources(
            ...
            experiment=exp,
            session=sess,
            options=["strip_metadata", "compare_raw_signals", "compare_time_coords", "print_channel_coord_summary"],
        )
        ...
        results["df_time"].to_csv(f"{out_path}df_raw_time_{sub}_{exp}_{sess}.csv", index=False)
    return results
...
def process_epoched_signals(
    ...
    *,
    ...
    def _paths_for(acq_tag: str):
        out_raw = os.path.join(out_path, f"df_epoch_{sub}_{exp}_{sess}_{acq_tag}.csv")
        out_raw_summary = os.path.join(out_path, f"df_epoch_summary_{sub}_{exp}_{sess}_{acq_tag}.csv")
        out_time = os.path.join(out_path, f"df_epoch_time_{sub}_{exp}_{sess}_{acq_tag}.csv")
        out_status = os.path.join(out_path, f"df_epoch_status_{sub}_{exp}_{sess}_{acq_tag}.csv")

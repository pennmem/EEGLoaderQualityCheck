...
class RawSignalPipeline(BasePipeline):
    ...
    def _output_paths(self) -> List[str]:
        ...
        if self.is_intracranial:
            paths = []
            for acq in ("mono", "bi"):
                paths.append(os.path.join(self.out_path, f"df_raw_{tag}_{acq}.csv"))
                paths.append(os.path.join(self.out_path, f"df_raw_summary_{tag}_{acq}.csv"))
                paths.append(os.path.join(self.out_path, f"df_time_{tag}_{acq}.csv"))
            return paths
        return [
            os.path.join(self.out_path, f"df_raw_{tag}.csv"),
            os.path.join(self.out_path, f"df_raw_summary_{tag}.csv"),
            os.path.join(self.out_path, f"df_raw_time_{tag}.csv"),
        ]
